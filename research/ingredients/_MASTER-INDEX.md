...
- **By relevance tier:**
  ...
  - Supporting: 96
  - Fringe: 81
- **Dossiers already written:** 40 dossier slugs are represented in the list below (flagged `Dossier? = ✅`), now including `dej-junction-peptides` (DEJ / basement-membrane "junction peptides" — see the dossier's key finding that Syn-Hycan, Palmitoyl Tetrapeptide-10/Crystalide, and mini-EGF are each *mislabeled* as DEJ actives; the true junctional peptides are laminin/Pal-RGD/Decorinyl; target biology is strong but human topical proof is weak; SKIP mini-EGF), `juveleven-hexapeptide-51` (Acetyl Hexapeptide-51 Amide / Juveleven — FOXO3a-mimetic "longevity" peptide; evidence is single-supplier in-vitro, no human RCT — conditional verdict), `next-gen-uv-filters` (modern broad-spectrum organic filters — bemotrizinol/Tinosorb S, bisoctrizole/Tinosorb M, DHHB, EHT, Mexoryl SX/XL/400, TriAsorB; note bemotrizinol became US-GRASE on 2026-06-09, the first new US filter since 1999), `titanium-dioxide` (mineral UV filter; evidence-based photoprotection but OTC-drug regulatory burden), `edelweiss-extract` (alpine antioxidant botanical / callus-culture "plant stem cell"), `cholesterol` (the cholesterol-dominant barrier-lipid dossier — co-star of the ceramide barrier story), `senocelltec` (alpine rose + apple/grape stem-cell senolytic blend), `eterwell-youth-epilobium` (botanical senolytic), `idebenone`, `vitamin-e-tocopherols` and the two cross-linked HPR files — `hydroxypinacolone-retinoate` (the active-molecule / literature view) and `granactive-retinoid` (the trade-blend / formulation view). All other ingredients are `❌` (no dossier yet).
